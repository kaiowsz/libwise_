...
import { getBooks, getCategories, getPopularBooks } from "@/actions/book";
import Link from "next/link";
import Pagination from "@/components/Pagination";
import CategoryFilter from "@/components/CategoryFilter";

export default async function Home({ searchParams }: { searchParams: Promise<{ page?: string; category?: string }> }) {
  const { page, category } = await searchParams;
  const currentPage = Math.max(1, parseInt(page || "1", 10) || 1);
  const categoryId = category || undefined;

  const [{ books, totalPages, currentPage: pg, totalCount }, categories] = await Promise.all([
    getBooks({ page: currentPage, categoryId }),
    getCategories(),
  ]);
  const popularBooks = await getPopularBooks();

  const isFirstPageNoFilter = currentPage === 1 && !categoryId;
  const mainBook = isFirstPageNoFilter && books.length > 0 ? books[0] : null;
  const gridBooks = isFirstPageNoFilter ? books.slice(1) : books;

  // Build base path for pagination (preserve category filter)
  const paginationBase = categoryId ? `/?category=${categoryId}` : "/";

  if (totalCount === 0 && !categoryId) {
    return (
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-6 py-16">
        ...
        {/* Featured Hero Book (only page 1, no filter) */}
        {mainBook && (
          <section className="flex flex-col md:flex-row gap-16 items-center md:items-start justify-between mb-32">
            <div className="flex-1 space-y-6 max-w-2xl">
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm md:text-base font-medium text-text-dimmed">
                ...
                {mainBook.category && <span>Category <span className="text-white">{mainBook.category.name}</span></span>}
              ...
              <div className="flex items-center gap-8 text-sm md:text-base text-text-dimmed pt-4 border-t border-border-dim/50">
                <span>Total Books in Library: <span className="text-white font-semibold">{totalCount}</span></span>
        ...
        {/* Popular Books Grid */}
        {popularBooks.books.length > 0 && !categoryId && (
              ...
              {popularBooks.books.map((book: any) => (
                <Link href={`/book/${book.id}`} key={book.id} className="relative group aspect-[3/4] rounded-sm overflow-hidden border border-border-dim hover:border-primary-500/50 transition-all shadow-lg hover:shadow-primary-500/20 transform hover:-translate-y-2 block">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-start justify-end p-4 gap-1">
                    ...
                    <p className="text-gray-400 font-semibold text-xs leading-tight">{book.author} {book.pages > 0 && `• ${book.pages} pages`}</p>
                    {book.category && <span className="text-xs text-primary-500 font-medium">{book.category.name}</span>}
                  </div>
                </Link>
              ))}
            </div>
        ...
        {gridBooks.length > 0 && (
          <section className="space-y-8 mt-10">
            <h2 className="text-2xl font-bold tracking-widest text-white uppercase flex items-center gap-4">
              {categoryId ? "Livros" : mainBook ? "Latest Books" : "Todos os Livros"}
              <div className="flex-1 h-px bg-border-dim"></div>
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 md:gap-8">
              {gridBooks.map((book: any) => (
                <Link href={`/book/${book.id}`} key={book.id} className="relative group aspect-[3/4] rounded-sm overflow-hidden border border-border-dim hover:border-primary-500/50 transition-all shadow-lg hover:shadow-primary-500/20 transform hover:-translate-y-2 block">
                  <Image 
                    src={book.coverUrl || "https://images.unsplash.com/photo-1542831371-29b0f74f9713?q=80&w=300&auto=format&fit=crop"} 
                    alt={book.title}
        ...
        {totalCount === 0 && categoryId && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-text-dimmed text-lg mb-4">Nenhum livro nesta categoria.</p>
